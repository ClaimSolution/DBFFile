...
async function openDBF(path: string, opts?: OpenOptions): Promise<DBFFile> {
    ...
    try {
        ...
        if (options.readMode !== "loose" && !isValidFileVersion(fileVersion)) {
            throw new Error(
                `File '${path}' has unknown/unsupported dBase version: ${fileVersion}.`
            );
        }
        ...
        if (fileVersion === 0x83 || fileVersion === 0x8b) {
            for (const ext of [".dbt", ".DBT"]) {
                ...
                let foundMemoFile =
                    (await stat(memoPath).catch(() => "missing")) !== "missing";
        ...
        if (fileVersion === 0x30) {
            ...
            const memoExt =
                dbExt == ".dbf" ? ".fpt" : `.${dbExt.substr(1, 2)}t`;
        ...
        while (headerLength > 32 + fields.length * 32) {
            ...
            let field: FieldDescriptor = {
                name: iconv
                    .decode(buffer.slice(0, 10), "ISO-8859-1")
                    .split("\0")[0],
                type: String.fromCharCode(
                    buffer[0x0b]
                ) as FieldDescriptor["type"],
            ...
            if (options.readMode !== "loose") {
                ...
                assert(
                    fields.every((f) => f.name !== field.name),
...
async function createDBF(
    ...
    try {
        ...
        let headerLength = 33 + fields.length * 32;
        ...
        result.dateOfLastUpdate = createDate(
            now.getFullYear(),
            now.getMonth() + 1,
            now.getDate()
        );
    ...
    try {
        ...
        if (dbf._memoPath) {
            ...
            if (dbf._version === 0x30) {
                ...
                memoBlockSize =
                    (dbf._version === 0x8b ? buffer.readInt32LE(0) : 0) || 512;
            }
        ...
        let currentPosition =
            dbf._headerLength + recordLength * dbf._recordsRead;

        // Create convenience functions for extracting values from the buffer.
        let substrAt = (start: number, len: number, enc: string) =>
            iconv.decode(buffer.slice(start, start + len), enc);
        let int32At = (start: number, len: number) =>
            buffer.slice(start, start + len).readInt32LE(0);
        ...
        while (true) {
            ...
            let recordCountToRead =
                maxRecords1 < maxRecords2 ? maxRecords1 : maxRecords2;
                    ...
                    switch (field.type) {
                        ...
                        case "T": // DateTime
                            if (buffer[offset] === 0x20) {
                                ...
                                const msSinceMidnight =
                                    buffer.readInt32LE(offset + 4) + 1;
                                value = parseVfpDateTime({
                                    julianDay,
                                    msSinceMidnight,
                                });
                            }
                            ...
                            while (true) {
                                ...
                                // Handle first/next block of dBase IV memo data.
                                    ...
                                    let isFirstBlockOfMemo =
                                        memoBuf.readInt32LE(0) === 0x0008ffff;
                                ...
                                if (blockIndex * memoBlockSize > memoFileSize) {
                                    throw new Error(
                                        `Error reading memo file (read past end).`
                                    );
                                }
                            }
                            break;
                        ...
                        default:
                            ...
                            if (dbf._readMode === "strict")
                                throw new Error(
                                    `Type '${field.type}' is not supported`
                                );
...
async function appendRecordsToDBF(
    dbf: DBFFile,
    ...
    try {
        ...
        let currentPosition =
            dbf._headerLength + dbf.recordCount * recordLength;
                ...
                switch (field.type) {
                    ...
                    case "F": // Float - appears to be treated identically to Number
                        ...
                        iconv
                            ...
                            .copy(buffer, offset, 0, field.size);
                        offset += field.size;
                        break;
                    ...
                    case "T": // DateTime
                        if (!value) {
                            iconv
                                .encode("        ", encoding)
                                .copy(buffer, offset, 0, 8);
                        } else {
                            const { julianDay, msSinceMidnight } =
                                formatVfpDateTime(value);
...
function validateFieldDescriptors(
    fields: FieldDescriptor[],
    fileVersion: number
): void {
...
function validateRecord(
    ...
    for (let i = 0; i < fields.length; ++i) {
        ...
        if (type === "C") {
            ...
            if (value.length > 255)
                throw new Error(
                    `${name}: text is too long (maximum length is 255 chars)`
                );
